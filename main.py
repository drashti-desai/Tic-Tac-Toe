...
def draw_lines(color=WHITE):
    for i in range(1, BOARD_ROWS):
        pygame.draw.line(surface=screen, color=color, start_pos=(0, SQUARE_SIZE * i), end_pos=(WIDTH, SQUARE_SIZE * i), width=LINE_WIDTH)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            if available_square(mouseY, mouseX):
                ...
                if check_win(player):
                    game_over = True
                player = player % 2 + 1 # this is for change the player
